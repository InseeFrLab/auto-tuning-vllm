...
class VLLMCLIParser:
    ...
    def _detect_argument_type(
        ...
        if (
            arg_name.startswith("--enable-")
            or arg_name.startswith("--disable-")
            or arg_name.startswith("--no-")
            or "enable" in description.lower()
        ...
        if (
            ...
            or "ratio" in description.lower()
            or "factor" in description.lower()
        ):
    ...
    def _parse_argument_line(self, line: str, section: str) -> Optional[CLIArgument]:
        ...
        arg_type = self._detect_argument_type(
            long_name, clean_description, default_value, choices
        )
    ...
    def get_arguments_by_section(self, section: str) -> List[CLIArgument]:
        ...
        return [
            ...
            for arg_name in self.sections[section]
    ...
    def generate_parameter_schema(
        ...
        for section in sections:
            for arg in self.get_arguments_by_section(section):
                ...
                param_def = {"description": arg.description}
    ...
    def export_defaults_yaml(
        self, output_path: str, sections: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        for section in sections:
            ...
            for arg in self.get_arguments_by_section(section):
                ...
                if not arg.long_name.startswith("--") or arg.long_name in [
                    "--help",
                    "--config",
                ]:
                    continue
    ...
    def export_versioned_defaults(
        self, base_dir: str, sections: Optional[List[str]] = None
    ) -> str:
        """
...
def main():
    ...
    try:
        ...
        print(
            f"\nFound {len(arguments)} arguments across "
            f"{len(parser.get_all_sections())} sections:"
        )
