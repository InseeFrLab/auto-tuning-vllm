...
| `data_column_mapper` | Column mappings injected into `generative_column_mapper` (supports both flat mapping and nested `column_mappings`) |
| `data_preprocessors` | Ordered list, include `"generative_column_mapper"` before `"flatten_image_lists"` / `"encode_media"` |
