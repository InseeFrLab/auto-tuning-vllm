...
| `data_column_mapper` | GuideLLM mapper config. Use `kind: "generative_column_mapper"` and `column_mappings` to map JSONL fields (e.g. `text_column: prompt`, `image_column: image`) |
