...
When `true` (default), the study skips any trial whose exact parameter combination has already been used in the study (completed, running, failed, or pruned optimization trials). Duplicate suggestions are rejected immediately and the sampler is asked again. If the search space is exhausted (e.g. grid search or a converged sampler), the study stops early after 100 consecutive duplicate suggestions. Set to `false` to allow re-running identical configurations.
